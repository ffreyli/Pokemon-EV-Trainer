...
const UpdatePokemon = (props) => {
    ...
    useEffect(() => {
        ...
        .then((response) => {
            ...
            // Set the search query to the current species name
            const speciesNumber = response.data.pokemonSpeciesNumber;
            const species = allPokemonSpecies.find(s => s.speciesNumber === speciesNumber);
            if (species) {
